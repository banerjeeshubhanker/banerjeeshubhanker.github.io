...
$$
\mathcal{R}(f) = \mathbb{E}_{(X, Y) \sim P} \left[ L(Y, f(X)) 
ight] = \int_{\mathcal{X} 	imes \mathcal{Y}} L(y, f(x)) \, dP(x, y).
$$
...
$$
\mathcal{R}(f) = \mathbb{E}_{(X, Y) \sim P} \left[(Y - f(X))^2
ight].
$$
...
$$
f^*(X) = rg\max_{k \in \mathcal{Y}} P(Y = k \mid X).
$$
